...
def _pseudo6(src, dst, length, proto):
    return src + dst + struct.pack("!I3xB", length, proto)


def icmpv6(src, dst, icmp_type, code, body):
...
def neighbor_advert(our_mac, our_ip, dst_mac, dst_ip, solicited):
...
def tcp6(our_mac, our_ip, dst_mac, dst_ip, sport, dport, seq, ack, flags):
...
def inspect(frame):
    """Parse an Ethernet frame into a dict of the fields the probe cares about, plus a one-line 'text'."""
    info = {"dst_mac": bytes(frame[0:6]), "src_mac": bytes(frame[6:12]), "type": "other"}
    if len(frame) < 14:
