...
def dhcp_frame(options):
    bootp = struct.pack("!BBBB4sH2s4s4s4s4s16s64s128s", 1, 1, 6, 0, b"\x12\x34\x56\x78", 0, b"\x80\x00",
...
def neighbor_solicitation(src, target):
    ...
    return pk.ethernet(pk.ipv6_multicast_mac(dst), MAC_HW, pk.ETH_IPV6,
...
class BridgeTests(unittest.TestCase):
    ...
    def test_neighbor_solicitations(self):
        ...
        self.assertTrue(pk.inspect(self.fn.sent[-1])["text"].endswith(
            f"neighbor advertisement for {pk.ip_str(win_ll)}"))
    ...
    def test_windows_packets_get_the_right_ethernet_destination(self):
        ...
        self.assertEqual(b.to_ethernet(pk.ipv4(WIN, bytes([224, 0, 0, 251]), 17, bytes(8)))[:6],
                         bytes.fromhex("01005e0000fb"))
        mdns6 = bytes.fromhex("ff0200000000000000000000000000fb")
        self.assertEqual(b.to_ethernet(pk.ipv6(pk.link_local(HOST_HW), mdns6, 17, bytes(8)))[:6],
                         bytes.fromhex("3333000000fb"))
